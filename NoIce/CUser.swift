...
class User{
  ...
  func BuscarNuevosMSG(userDestino: CKRecord.ID) {
    
    let predicateMesajes = NSPredicate(format: "to == %@ and from ==%@",userDestino,recordID)
    
    let queryMSGVista = CKQuery(recordType: "Mensaje",predicate: predicateMesajes)
  ...
  func ActualizarBloqueo(userBloqueado: String, completionHandler: @escaping(Bool)->()){
    self.bloqueados.append(userBloqueado)
